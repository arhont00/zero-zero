...
@router.callback_query(F.data == "admin_diag_all")
async def admin_diag_all(callback: CallbackQuery):
    """Список всех заявок диагностики."""
    items = DiagnosticModel.get_all(limit=50)
    if not items:
        await callback.message.edit_text(
            "📭 Заявок пока нет.",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    text = "📋 *ВСЕ ЗАЯВКИ НА ДИАГНОСТИКУ*\n\n"
    buttons = []
    for d in items:
        text += (
            f"🆔 #{d['id']} | {d['first_name']} (@{d['username']})\n"
            f"📅 {format_datetime(d['created_at'])}\n"
            f"📊 Статус: {'✅ обработано' if d['sent'] else '⏳ новое'}\n\n"
        )
        buttons.append([InlineKeyboardButton(text=f"👁️ Заявка #{d['id']}", callback_data=f"diag_view_{d['id']}")])
...
@router.callback_query(F.data.startswith("diag_contact_"))
async def admin_diag_contact(callback: CallbackQuery):
    """Вспомогательная карточка контакта клиента диагностики."""
    diag_id = int(callback.data.replace("diag_contact_", ""))
    with db.cursor() as c:
        c.execute("""
    ...
    await callback.message.edit_text(
        f"✍️ *Связь с клиентом диагностики #{diag_id}*\n\n"
        f"👤 {name}\n"
        f"🆔 `{row['user_id']}`\n\n"
        f"Нажмите кнопку ниже, чтобы открыть чат.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📞 ОТКРЫТЬ ЧАТ", url=f"tg://user?id={row['user_id']}")],
            [InlineKeyboardButton(text="🔙 К ДИАГНОСТИКЕ", callback_data=f"diag_view_{diag_id}")]
        ]),
        parse_mode="Markdown"
    )
