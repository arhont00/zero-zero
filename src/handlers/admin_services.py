...
@router.callback_query(F.data.startswith("admin_service_edit_"))
async def admin_service_edit(callback: CallbackQuery, state: FSMContext):
    """Меню редактирования услуги."""
    service_id = int(callback.data.replace("admin_service_edit_", ""))
    service = ServiceModel.get_by_id(service_id)
    if not service:
        await callback.answer("❌ Услуга не найдена")
        return
    ...
    buttons = [
        [InlineKeyboardButton(text="📝 Название", callback_data=f"admin_service_edit_field_{service_id}_name")],
        [InlineKeyboardButton(text="📄 Описание", callback_data=f"admin_service_edit_field_{service_id}_description")],
        [InlineKeyboardButton(text="💰 Цена", callback_data=f"admin_service_edit_field_{service_id}_price")],
        [InlineKeyboardButton(text="⏱️ Длительность", callback_data=f"admin_service_edit_field_{service_id}_duration")],
        [InlineKeyboardButton(text="🔙 К УСЛУГЕ", callback_data=f"admin_service_view_{service_id}")]
    ]
    await callback.message.edit_text(
...
@router.callback_query(F.data.startswith("admin_service_edit_field_"))
async def admin_service_edit_field_pick(callback: CallbackQuery, state: FSMContext):
    """Выбор поля редактирования услуги."""
    # Формат: admin_service_edit_field_{id}_{field}
    ...
    field = parts[5]
    if field not in {"name", "description", "price", "duration"}:
        await callback.answer("❌ Неверное поле")
        return

    await state.update_data(service_edit_id=service_id, service_edit_field=field)
    await state.set_state(AdminServiceStates.service_edit_field)
    prompts = {
        "name": "Введите новое название услуги:",
        "description": "Введите новое описание услуги:",
        "price": "Введите новую цену (число):",
        "duration": "Введите новую длительность в минутах (число):",
    }
    await callback.message.edit_text(prompts[field])
    await callback.answer()


@router.message(AdminServiceStates.service_edit_field)
async def admin_service_edit_field_save(message: Message, state: FSMContext):
    """Сохранение измененного поля услуги."""
    data = await state.get_data()
    service_id = data.get("service_edit_id")
    field = data.get("service_edit_field")
    ...
    if field == "price":
        ...
        try:
            ...
            await message.answer("❌ Введите длительность числом")
            return

    with db.cursor() as c:
        c.execute(f"UPDATE services SET {field} = ? WHERE id = ?", (value, service_id))
