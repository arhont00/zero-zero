...
@router.callback_query(F.data.startswith("admin_club_activate_"))
async def admin_club_activate(callback: CallbackQuery):
    """Ручная активация платной подписки пользователю на 30 дней."""
    if not UserModel.is_admin(callback.from_user.id):
        await callback.answer("❌ Нет прав")
        return

    user_id = int(callback.data.replace("admin_club_activate_", ""))
    payment_id = f"manual_admin_{callback.from_user.id}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ok = ClubModel.activate_paid(user_id=user_id, payment_id=payment_id, duration_days=30)
    if not ok:
        await callback.answer("❌ Не удалось активировать подписку", show_alert=True)
        return

    await callback.message.edit_text(
        f"✅ Подписка активирована для пользователя {user_id} на 30 дней.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 К ПОЛЬЗОВАТЕЛЮ", callback_data=f"admin_club_user_{user_id}")],
            [InlineKeyboardButton(text="🔙 К СПИСКУ", callback_data="admin_club_list")]
        ])
    )
    await callback.answer()
