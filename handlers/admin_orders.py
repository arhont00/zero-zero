...
async def contact_user(callback: CallbackQuery):
    """Показать быстрый контакт клиента заказа."""
    order_id = int(callback.data.replace("contact_user_", ""))
    order = OrderModel.get_by_id(order_id)
    if not order:
        await callback.answer("❌ Заказ не найден")
        return

    name = order['first_name'] or order['username'] or str(order['user_id'])
    await callback.message.edit_text(
        f"📞 *Клиент заказа #{order_id}*\n\n"
        f"👤 {name}\n"
        f"🆔 `{order['user_id']}`\n\n"
        f"Нажмите кнопку ниже, чтобы открыть чат с клиентом.",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✍️ ОТКРЫТЬ ЧАТ", url=f"tg://user?id={order['user_id']}")],
            [InlineKeyboardButton(text="🔙 К ЗАКАЗУ", callback_data=f"order_view_{order_id}")]
        ]),
        parse_mode="Markdown"
    )
